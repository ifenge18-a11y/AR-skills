...
本仓库用于维护会计学研究相关的 Codex Skill。目前包含：

- [`accounting-research-assistant`](./accounting-research-assistant)：面向会计学、财务管理和公司金融研究的研究辅助 Skill。

## Accounting Research Assistant

当前版本：`v0.1.0`

`accounting-research-assistant` 用于辅助研究者完成从选题构思、文献理解、理论分析、实证设计、数据规划、代码方案到论文写作与修改的研究流程。

适用场景包括会计学术论文、A 股实证研究、审计研究、信息披露研究、财务报告质量研究、公司治理研究、ESG 与可持续披露研究、资本市场会计研究、论文文本修改和研究计划制定。

## 功能边界

本仓库当前保持单一 skill 结构，不拆分为多个独立 skill。`accounting-research-assistant` 聚焦于会计、金融和财务管理研究本身，不处理做图、投稿发表策略、审稿回复、PPT 汇报、Nature/CNS 风格泛科研写作或生物医学工作流。
...
Boss 默认采用阶段式并行：先启动文献综述角色，在初步文献结果稳定后并行推进理论分析和实证设计，再依次进入代码实现和写作整合。Boss 最后必须指出各角色输出中的逻辑漏洞、识别风险、文献缺口和写作问题。
...
本仓库已公开。其他用户可以 fork 仓库并通过 Pull Request 提交修改建议，由仓库维护者审核后合并。
